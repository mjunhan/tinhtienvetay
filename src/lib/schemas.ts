...
export const productSchema = z.object({
    ...
    negotiated_price_cny: z.preprocess((val) => (typeof val === 'number' && isNaN(val) ? undefined : val), z.number().min(0).optional()),
    weight_kg: z.preprocess((val) => (typeof val === 'number' && isNaN(val) ? undefined : val), z.number().min(0).optional()), // Can match 0 if using total weight
});
...
export const calculatorSchema = z.object({
    ...
    internal_ship_cny: z.preprocess((val) => (typeof val === 'number' && isNaN(val) ? undefined : val), z.number().min(0).optional()),
